...
def register(request):
    if request.method == "POST":
        form = GuestStudentRegistrationForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(
                request,
                "Account request submitted. You can sign in after an admin approves it.",
            )
        ...
        form = GuestStudentRegistrationForm()
...
def admin_panel(request):
    ...
    return render(
        ...
        {"title": "Admin Panel", "pending_users_count": pending_users_count},
    )
...
def pending_users(request):
    users = User.objects.filter(is_active=False, is_superuser=False).order_by(
        "-date_joined"
    )
    return render(
        request,
        "accounts/pending_users.html",
        {"title": "Pending User Approvals", "users": users},
    )
...
def reject_user(request, pk):
    ...
    user.delete()
    messages.success(request, f"{full_name}'s signup request has been rejected.")
    return redirect("pending_users")
